...
\thanks{Both authors are independent researchers. Corresponding author: V. Varadharajan (vv0366@srmist.edu.in).}}

\maketitle

\begin{abstract}
Retrieval-Augmented Generation relocates a system's factual authority into a corpus an
adversary may be able to write to. Existing defenses require
LLM decoder access, multiplicative inference overhead, or cryptographic
pre-registration, none of which offline on-device deployment can pay. We present SEVA,
a fully local, LLM-free detector built on one geometric signal, per-document
K-nearest-neighbour cluster coherence, operated as a hard gate against templated
multi-passage poisoning. In-domain, where clean and poison share a security domain so
no topic shortcut exists, SEVA drives templated poison-evasion to zero across three
seeds at a 0.56 percent document-level false-positive rate under frozen, non-oracle
calibration, and catches PoisonedRAG's released poison at 82 to 98 percent across three
corpora. End to end the gate removes all the corruption it detects. We then delimit the
assumption it rests on. Host-anchored cloning evades the gate on every target while
remaining retrievable, no complementary geometric signal we tested closes the gap, and
the bypass is cheap. Reproducing CleanBase at a matched operating point yields
the same pattern, placing the boundary in the mutual-similarity assumption the
detection family shares rather than in our statistic. Detection is encoder-invariant,
reproducible across backends, flat to one million documents, at 13 to 38 milliseconds
per query.
\end{abstract}
...
take RAGDefender as our head-to-head baseline: we reproduce its published filter to obtain
its benign strip rate on a shared corpus and attack, and compare detection against a per-query
ablation of cluster coherence at matched false-positive rate.
...
\midrule
CleanBase~\cite{cleanbase}   & similarity cliques & $\checkmark$ & $\checkmark$ & $\checkmark$ \\
SeCon-RAG~\cite{secon}       & centroid tightness & $\times$ & $\times$ & $\times$ \\
GRADA~\cite{grada}           & centrality rerank & $\times$ & $\checkmark$ & $\times$ \\
TrustRAG~\cite{trustrag}     & $k$-means + LLM check & $\times$ & $\times$ & $\times$ \\
TopoGuard~\cite{topoguard}   & $k$-NN spectral gap & $\times$ & $\checkmark$ & $\checkmark$ \\
\textbf{SEVA (ours)}         & $K$-NN cohesion & $\checkmark$ & $\checkmark$ & $\checkmark$ \\
\midrule
Hubness~\cite{hubness}       & retrieval frequency & $\checkmark$ & $\checkmark$ & $\checkmark$ \\
RAGDefender~\cite{ragdefender}& per-window density & $\times$ & $\checkmark$ & $\times$ \\
RobustRAG~\cite{robustrag}   & isolate-then-aggregate & $\times$ & $\times$ & $\times$ \\
AV Filter~\cite{avfilter}    & attention variance & $\times$ & $\times$ & $\times$ \\
RAGShield~\cite{ragshield}   & provenance & $\checkmark$ & $\checkmark$ & $\checkmark$ \\
\bottomrule
\end{tabular}
\end{table}
...
$\mathrm{FPR}_{\mathrm{TARGET}}=0.69\%$. This uses no labelled poison, no poison scores and no
density estimate---which is why the same $\tau$ transfers unchanged to held-out poison
(\S\ref{subsec:geom}). It does take the quantile over the benign partition, which our harness
knows by construction; a deployment would take it over the scores its benign query stream
returns, which needs no labels at all. We measured that estimator directly: at $1\%$ and $5\%$
contamination it recovers the frozen threshold ($\tau=0.844$--$0.848$ against $0.8423$) at $0\%$
poison-evasion on every seed. In-domain this places the geometric gate at
...
neighbourhoods, the quantity the supplement reports, is $\approx\!0.75$).
...
construction---our re-implementation, since no poison is published for this domain (used in
the cross-corpus comparison, \S\ref{subsec:lex}). On the
...
(supplement): in-domain, clean documents sit at coherence $\approx 0.75$
...
(supplement); neither quantity shows systematic density dependence. It is
...
\end{axis}
\end{tikzpicture}
\caption{Calibration scaling and density-invariance across corpus size. Realized non-oracle document-FPR against the $0.69\%$ target at $10$k, $100$k and $1$M: the deviation does not grow with $N$ (Observation~2). The per-density coherence gap behind it is tabulated in the supplement. Density-invariance is a large-corpus property, and we plot where it fails rather than omitting it.}
\label{fig:scaling}
\end{figure}
...
loose---$12$--$18\times$ wider than the realized deviation at every $N$---so we report it for
the mechanism, not as a numerical prediction: the deviation is a uniform-convergence quantity
bounded by the calibration sample alone. The measured points shrink faster than the bound
requires at the first decade and slower at the second, so we claim the direction, not a rate.

\emph{Scope.} Established at three corpus sizes spanning two decades. Because the largest
point also uses a broader corpus (\S\ref{subsec:scale}), we report the direction, fit no
exponent, and do not extrapolate beyond $10^{6}$.
...
offline LLM (\texttt{mistral:7b-instruct}; \texttt{gpt-oss:20b} loops on the rewrite prompt) to
preserve meaning while changing surface form, then splices in the payload. Each
...
another---the mutual similarity the attack exists to avoid (the supplement reports the
...
lacks: a separation invariant to how large that cluster grows (supplement), and
...
matched point reproduces that boundary exactly, placing it in the shared mutual-similarity
assumption rather than in our statistic. That boundary---not the signal---is the contribution:
knowing where a defense stops is what lets an operator deploy it deliberately. Detecting
injections that leave no duplication footprint, without an LLM, remains open.
...
\textbf{Use of AI in the experiments:} Two locally hosted open-weight models generated and judged experimental data; neither is part of the detector. \texttt{mistral:7b-instruct} rewrote host-anchored clone passages (\S\ref{subsec:boundary}); \texttt{gpt-oss:20b} at temperature $0$ judged answer corruption end to end. Data so generated is labelled as such in the released artifacts. SEVA itself calls no language model.
\textbf{Financial support:} This research received no external grant funding. \textbf{Conflicts of interest:} The authors declare none. \textbf{Data and code availability:} The frozen detector, reproduction toolkit, generated clone datasets, and a result file behind every headline number are available at \url{https://github.com/varadharajanv0310/SEVA-IEEE}; corpora and poison are gated by order-sensitive SHA-256 hashes recorded in each result file, so a reproduction that does not match fails closed rather than diverging silently.
...
\bibitem{avfilter} S. Choudhary, N. Palumbo, A. Hooda \emph{et al.}, ``Through the stealth lens: Rethinking attacks and defenses in RAG,'' \emph{arXiv preprint} arXiv:2506.04390, 2025.
...
\bibitem{bge} S. Xiao, Z. Liu, P. Zhang, and N. Muennighoff, ``C-Pack: Packed resources for general Chinese embeddings,'' in \emph{Proc. 47th Int. ACM SIGIR Conf. on Research and Development in Information Retrieval}, 2024, pp. 641--649, arXiv:2309.07597.
\bibitem{faiss} M. Douze, A. Guzhva, C. Deng \emph{et al.}, ``The FAISS library,'' \emph{arXiv preprint} arXiv:2401.08281, 2024.
