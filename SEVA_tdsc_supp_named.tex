...
(Section~\ref{app:compare}), supporting detection tables (Section~\ref{app:tables}), the
signal inventory behind the composite ablation (Section~\ref{app:signals}),
the geometric core against that composite (Section~\ref{app:core}),
cross-platform reproduction (Section~\ref{app:xplat}), and per-condition
detail for four results the main paper states in summary form
(Section~\ref{app:detail}). Table
...
$3.2\times$ at no cost to catch rate.}
\label{tab:agg}
\begin{tabular}{lcc}
\toprule
...
\begin{tabular}{lcccc}
\toprule
Platform & Gap ($1$--$10\%$) & SNR & Evasion & Latency (ms) \\
\midrule
RTX 5080 (CUDA) & $+0.236$--$0.245$ & $5.9$--$6.1$ & $\mathbf{0\%}$ & $13.4$--$15.7$/$18.8$ \\
RTX 4060 (CUDA) & $+0.236$--$0.245$ & $5.9$--$6.1$ & $\mathbf{0\%}$ & $38.1$/$43.6$ \\
Apple M4 (MPS) & $+0.236$--$0.245$ & $5.9$--$6.1$ & $\mathbf{0\%}$ & $28.1$/$37.3$ \\
\bottomrule
\end{tabular}
\end{table}

\section{Per-Condition Detail}
\label{app:detail}
This section collects the per-condition detail behind four results the main paper
states in summary form: the geometric gate under three embedding lineages
(Observation~3), the reproduced shared-corpus matched-FPR head-to-head, the
million-document scale validation, and cross-domain detection against lexical
MinHash and embedding near-duplicate detection. Every row is calibrated non-oracle
on clean coherence alone, so no row borrows information from the poison it is
scored against.
...
\begin{tabular}{lccccc}
\toprule
Encoder & Clean & Poison & Gap & SNR & Evasion \\
\midrule
bge-large (BAAI) & $0.751$ & $0.990$ & $+0.236$--$0.245$ & $5.9$--$6.1$ & $\mathbf{0\%}$ \\
e5-large-v2 (Microsoft) & $0.866$ & $0.990$ & $+0.122$--$0.127$ & $6.4$--$6.7$ & $\mathbf{0\%}$ \\
gte-large (Alibaba) & $0.881$ & $0.996$ & $+0.113$--$0.117$ & $5.2$--$5.3$ & $\mathbf{0\%}$ \\
\bottomrule
\end{tabular}
\end{table}
...
(the encoder-invariance section of the main paper, the cross-platform section of the main paper) report an independently re-embedded measurement of
