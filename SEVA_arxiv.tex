...
\emph{Scope.} This is a measurement at three corpus sizes ($10{,}000$, $100{,}000$,
$1{,}000{,}000$), not a rate result: the deviation is smaller at $10^{6}$ than at $10^{4}$, and
that is the whole of the claim. The largest point also uses a broader corpus
(\S\ref{subsec:scale}), so we report the direction, fit no exponent, and do not extrapolate.
...
The gate does not see this attack. At one clone per target---the configuration the attack is
built for, since each passage is anchored to a \emph{different} host---clone cohesion averages
$0.751$ against $\tau=0.841$, below the gate for \emph{every} one of $50$ targets ($100\%$
poison-evasion), while $84\%$ remain retrieved at $K=5$. Multiplicity rescues detection only
partially, and at the attacker's own expense: raising the injection from $1$ to $5$ clones per
target moves mean cohesion from $0.722$ to $0.790$ and the gate's catch rate from $0\%$ to
$22.7\%$ (evasion $100\%\to77.3\%$), because clones that share a payload start to resemble one
another---the mutual similarity the attack exists to avoid. The adversary's best configuration
is therefore the single anchored clone, and that is the one the gate never sees. Nor does the complementary signal: nearest-neighbour
