...
% allow floats to occupy more of a page so section-local placement does not flush pages
...
\title{SEVA: Lightweight, LLM-Free Detection of Templated Corpus Poisoning in Retrieval-Augmented Generation, and the Boundary of Cohesion-Based Defense\\[2pt]
\large\normalfont(Extended Version)}

\author{V.~Varadharajan and Abishek~V.~P.~T.%
\IEEEcompsocitemizethanks{\IEEEcompsocthanksitem Both authors are independent researchers.}}
...
encounters at the frozen gate---at a 0.56\% document-level false-positive rate, under
...
all the corruption it detects: the templated attack corrupts 22\% of targets undefended and 0\%
...
byte-identical, hash-verified corpus. Detection stays flat
...
discards roughly half the clean corpus. SEVA is deliberately scoped to one dominant attack
family---templated multi-passage injection---and defended across domains, adaptive adversaries,
encoders, and hardware; within that scope it is, to our knowledge, the first cohesion-based
detector whose operating envelope and failure boundary are both measured rather than assumed.
\end{abstract}
...
Questions and HotpotQA; a faithful black-box re-implementation of their released
construction on the Security corpus, for which no poison is published), while lexical
...
multi-passage poisoning at 0\% poison-evasion across three seeds and 0.56\% document-FPR under
...
take RAGDefender as our head-to-head baseline, reproducing its published filter to
obtain its benign strip rate on a shared corpus and attack, and comparing detection against a
per-query ablation of cluster coherence at matched false-positive rate.
...
The cohesion signal, then, is not ours to claim---but its operating envelope is, and
that is what none of these works establishes.
...
$3.2\times$ at no cost to detection.
...
SEVA's threshold is calibrated with no knowledge of the poison and no oracle over the
contamination density, in two phases over a fixed pool of benign queries. In the
\emph{calibration} phase $\tau$ is set to the $(1-\mathrm{FPR}_{\mathrm{TARGET}})$ quantile of
the coherence distribution of the corpus's benign documents, by 50-iteration binary search, with
$\mathrm{FPR}_{\mathrm{TARGET}}=0.69\%$. We state precisely what this does and does not assume.
It uses no labelled poison, no poison scores, and no knowledge of the density---the three things
an oracle calibration would exploit, and the reason the same $\tau$ transfers unchanged to
held-out poison (\S\ref{subsec:geom}). It does, in our harness, take the quantile over the
benign partition, which the harness knows by construction. A deployment has no such partition
and would instead take the quantile over the scores of documents returned by its benign query
stream, which needs no labels at all. We measured that estimator directly: at $1\%$ and $5\%$
contamination it recovers the frozen threshold ($\tau=0.844$--$0.848$ against the frozen
$0.8423$) at $0\%$ poison-evasion on every seed, so the deployment procedure and the harness
procedure agree where it matters. In-domain this places the geometric gate at
...
Table~\ref{tab:signals} gives the full signal inventory and its in-domain separation. The
...
receives a faithful black-box PoisonedRAG built to the authors' released
construction---a re-implementation, since no poison is published for this domain, with the
target question prepended to an LLM-crafted payload (used in the cross-corpus comparison,
\S\ref{subsec:lex}). On the
...
\centering
\begin{tikzpicture}
\begin{axis}[width=0.74\columnwidth, height=4.1cm, xbar, bar width=4.2mm,
  xmin=0.70, xmax=1.06, xlabel={\texttt{cluster\_coh}},
  ytick={0,1,2,3}, y dir=reverse,
  yticklabel style={font=\scriptsize},
  yticklabels={clean, clean (pois.), poison min, poison mean},
  nodes near coords, nodes near coords style={font=\tiny},
  point meta=explicit symbolic, tick label style={font=\scriptsize}, label style={font=\scriptsize}, legend style={font=\scriptsize, draw=none, fill=none}, grid=both, grid style={line width=.2pt, draw=gray!20}]
\addplot[fill=black!12, draw=black!45] coordinates {(0.7510,0) [0.7510] (0.7485,1) [0.7485] (0.8905,2) [0.8905] (0.9951,3) [0.9951]};
\draw[red!60!black, dashed, thick] (axis cs:0.8423,3.6)
  -- (axis cs:0.8423,-0.6)
  node[above, font=\tiny, red!60!black, anchor=south] {frozen $\tau=0.8423$};
\end{axis}
\end{tikzpicture}
\caption{Why zero evasion is not a knife-edge. Every poison document sits above the
frozen $\tau=0.8423$: the \emph{minimum} poison coherence over the whole run is $0.8905$, against
a clean mean of $0.7510$, so the gate has margin on its worst case rather than only on average.
Contaminating the corpus barely moves the clean mean ($0.7510\to0.7485$). Across $25{,}000$
high-encounter trials this yields $0$ evasions, a $95\%$ Wilson upper bound of $0.0154\%$.}
\label{fig:ci}
\end{figure}
...
Appendices~\ref{app:confusion} and~\ref{app:tables} (per-seed confusion matrices and per-condition rates).
...
\centering
\begin{tikzpicture}
\begin{axis}[width=\columnwidth, height=3.3cm, xmode=log, log basis x=10,
  xlabel={clean calibration corpus size $N$}, ylabel={doc-FPR (\%)},
  xmin=6000, xmax=1.8e6, ymin=0.6, ymax=0.84, xtick={1e4,1e5,1e6},
  legend pos=north east, tick label style={font=\scriptsize}, label style={font=\scriptsize}, legend style={font=\tiny, draw=none, fill=none}, grid=both, grid style={line width=.2pt, draw=gray!20}]
\addplot[thick, blue!60!black, mark=*] coordinates {(10000,0.7653) (100000,0.6741) (1000000,0.7008)};
\addlegendentry{realized}
\addplot[green!35!black, dash dot, thick, mark=none, domain=6000:1.8e6] {0.69};
\addlegendentry{$0.69\%$ target}
\end{axis}
\end{tikzpicture}
...
\begin{axis}[width=\columnwidth, height=3.3cm,
  xlabel={contamination density (\%)}, ylabel={coherence gap},
  xmin=0, xmax=11, ymin=0.11, ymax=0.42, xtick={1,5,10},
  legend pos=north west, legend columns=3, tick label style={font=\scriptsize}, label style={font=\scriptsize}, legend style={font=\tiny, draw=none, fill=none}, grid=both, grid style={line width=.2pt, draw=gray!20}]
\addplot[thick, black!45, mark=square*] coordinates {(1,0.1411) (5,0.2813) (10,0.2789)};
\addlegendentry{$N=10$k}
\addplot[thick, blue!60!black, mark=*] coordinates {(1,0.2363) (5,0.2408) (10,0.2445)};
\addlegendentry{$N=100$k}
\addplot[thick, orange!85!black, mark=triangle*] coordinates {(1,0.2454) (5,0.2489)};
\addlegendentry{$N=1$M}
\end{axis}
\end{tikzpicture}
\caption{Calibration scaling and density-invariance across corpus size. \emph{Top:} realized non-oracle document-FPR against the $0.69\%$ target at $10$k, $100$k and $1$M---the deviation does not grow with $N$ (Observation~2). \emph{Bottom:} the cluster-coherence gap per contamination density; the $100$k and $1$M curves are flat, while at $10$k the gap collapses at $1\%$ density. Density-invariance is a large-corpus property, and we plot where it fails rather than omitting it.}
\label{fig:scaling}
\end{figure}
...
realized false-positive rate at the plug-in threshold $\tau=F_n^{-1}(1-\alpha)$ satisfies
\[
  \bigl|\mathrm{FPR}(\tau)-\alpha\bigr| \;\le\; \sup_x \bigl|F_n(x)-F(x)\bigr|
  \;\le\; \bigl(\ln(2/\delta)/2n\bigr)^{1/2}
\]
with probability $1-\delta$, the second inequality being the
Dvoretzky--Kiefer--Wolfowitz bound~\cite{dkw}. The deviation therefore decays as
...
loose---$12$--$18\times$ wider than the realized deviation at every $N$ we ran---and the
three measured deviations do not follow the rate it predicts (successive ratios $4.74$ and
$1.47$, against the $3.16$ a $1/\sqrt{n}$ decay would give). We therefore report the bound as
the reason a deviation of this kind must shrink, and the measurements as what it does, without
claiming the data exhibit the rate.

\emph{Scope.} Established at three corpus sizes spanning two decades ($10{,}000$,
$100{,}000$, $1{,}000{,}000$). Because the largest point also uses a broader corpus
(\S\ref{subsec:scale}), we report the direction, fit no exponent, and do not extrapolate.
...
\centering
\begin{tikzpicture}
\begin{axis}[width=\columnwidth, height=4.1cm,
  xlabel={contamination density (\%)}, ylabel={\texttt{cluster\_coh} gap},
  xmin=0, xmax=11, ymin=0.09, ymax=0.42, xtick={1,5,10},
  legend pos=north west, legend columns=1, tick label style={font=\scriptsize}, label style={font=\scriptsize}, legend style={font=\scriptsize, draw=none, fill=none}, grid=both, grid style={line width=.2pt, draw=gray!20}]
\addplot[thick, blue!60!black, mark=*] coordinates {(1,0.2363) (5,0.2408) (10,0.2445)};
\addlegendentry{bge-large (SNR $\geq$ 5.93)}
\addplot[thick, orange!80!black, mark=square*] coordinates {(1,0.1223) (5,0.1246) (10,0.1266)};
\addlegendentry{e5-large-v2 (SNR $\geq$ 6.43)}
\addplot[thick, green!40!black, mark=triangle*] coordinates {(1,0.1130) (5,0.1150) (10,0.1166)};
\addlegendentry{gte-large (SNR $\geq$ 5.21)}
\end{axis}
\end{tikzpicture}
...
\label{subsec:lex}
The cross-domain result also answers the sharpest objection to a cohesion-based detector: that
...
retrieval window and removes a side, so on benign in-domain queries it strips $51.2\%$ of clean
...
\emph{queries} are flagged. Per-query aggregation removes it (Appendix~\ref{app:compare}).
...
are flagged cuts the benign query-level false-positive rate from $3.06\%$ to
$0.97\%$---a $3.2\times$ reduction, means over the nine in-domain conditions---while the
poison catch rate is unchanged at $98\%$.
...
The gate does not see this attack. Each clone is anchored to a \emph{different} host, so
one clone per target is the configuration the attack is built for---and there it is invisible:
clone cohesion averages $0.751$ against $\tau=0.841$, below the gate for \emph{every} one of
$50$ targets ($100\%$ poison-evasion), while $84\%$ remain retrieved at $K=5$. Raising the
injection to five clones per target is the only thing that moves the statistic, and it moves it
against the attacker rather than for him, since clones sharing a payload begin to resemble one
another---the mutual similarity the attack exists to avoid (Table~\ref{tab:multiplicity} reports the
sweep). The adversary's optimum is therefore the single anchored clone, and that is the one the
gate never sees. Nor does the complementary signal: nearest-neighbour
...
the gate does stop reaches $22\%$ undefended (\S\ref{subsec:e2e}). Sweeping payload
...
is deployed at all. Against an adversary willing to change strategy, then, the gate removes
the attack it detects without raising the attacker's price.
...
\subsection{The Boundary in Full}
\label{app:boundaryfull}
\begin{table}[H]
\centering
\caption{Injection multiplicity restores detection only partially, and at the attacker's expense. Host-anchored clones scored in the
...
the attack corrupts $22\%$ of the $50$ targets ($11/50$) and places $2.74$ poison passages in the average
...
\begin{axis}[width=\columnwidth, height=4.1cm,
  xlabel={contamination density (\%)}, ylabel={\texttt{cluster\_coh} gap},
  xmin=0, xmax=11, ymin=0.232, ymax=0.252, xtick={1,5,10},
  ytick={0.234,0.238,0.242,0.246,0.250},
  yticklabel style={/pgf/number format/.cd, fixed, precision=3},
  legend pos=south east, tick label style={font=\scriptsize}, label style={font=\scriptsize}, legend style={font=\scriptsize, draw=none, fill=none}, grid=both, grid style={line width=.2pt, draw=gray!20}]
\addplot[thick, blue!60!black, mark=*, mark size=2.6pt] coordinates {(1,0.2363) (5,0.2408) (10,0.2445)};
\addlegendentry{RTX 4060 (CUDA)}
\addplot[only marks, orange!85!black, mark=o, mark size=4pt, thick] coordinates {(1,0.2363) (5,0.2408) (10,0.2445)};
\addlegendentry{Apple M4 (MPS)}
\end{axis}
\end{tikzpicture}
...
The separation is a property of the attack's
...
\subsection{Deployment Cost of the Corpus-Level Precompute}
\label{app:cost}
...
It belongs behind controls that do not depend on the attacker's text---provenance on the
ingestion path, rate and origin limits on who may write to the corpus, and review of documents
entering through low-trust channels---and deployed there it is a precise instrument: a filter for
known attack structure, with a characterized envelope and a known edge. What it does not do is
bound the adversary, and an operator should size it accordingly, since an attacker who reads this
paper can evade it with one paraphrase per target. The geometric signal tells you when someone
has used the standard attack; it cannot tell you that nobody has attacked.
...
$22\%$ the templated attack achieves undefended. Neither injection multiplicity nor
...
\subsection{Negative Results, and One Retraction}
\label{app:negative}
...
$22\%$ for the templated attack it replaces---it is more effective, not less. The earlier claim
...
\subsection{What Would Close the Boundary}
\label{app:future}
...
$0.56\%$ document-FPR, catches PoisonedRAG's own poison at $82$--$98\%$ across three corpora,
...
expensive---it corrupts more targets than the attack it replaces. Reproducing CleanBase at a
matched operating point reproduces that boundary exactly, which places it in the shared
mutual-similarity assumption rather than in our statistic. Knowing where a defense stops is
what lets an operator deploy it deliberately, and that boundary---not the signal---is what this
paper contributes. Detecting injections that leave no duplication footprint, without an LLM in
...
cuts the benign query-level false-positive rate $3.2\times$ at no cost to catch rate.
...
$3.2\times$ at no cost to catch rate. Rates are means over the nine in-domain conditions.}
\label{tab:agg}
\begin{tabular}{lcc}
\toprule
Aggregation rule & Benign query-FPR & Poison catch \\
\midrule
$\geq\!1$ flagged & 3.06\% & 98.0\% \\
$\geq\!2$ flagged & \textbf{0.97\%} & \textbf{98.0\%} \\
\bottomrule
\end{tabular}
\end{table}
...
\label{app:detail}
The tables in this appendix give the per-condition detail behind every summary figure in the main text. Each is calibrated non-oracle on clean coherence alone, so no row borrows
information from the poison it is scored against.
...
\centering
\begin{tikzpicture}
\begin{axis}[width=\columnwidth, height=2.7cm,
  xlabel={false-positive rate (\%)}, ylabel={catch rate (\%)},
  xmin=0, xmax=5, ymin=97.5, ymax=100.6, ytick={98,99,100},
  tick label style={font=\scriptsize}, label style={font=\scriptsize},
  grid=both, grid style={line width=.2pt, draw=gray!20}]
...
\draw[green!35!black, dash dot] (axis cs:0.69,97.5) -- (axis cs:0.69,100.6);
\node[green!30!black, font=\tiny, anchor=west] at (axis cs:0.8,98.6) {deployed point};
\end{axis}
\end{tikzpicture}
...
Every number in this paper comes from one of three corpora, one frozen constant set, and three software environments; all are listed here so a reader can tell at a glance what was held fixed and what was varied.
...
\midrule
Security SE      & $100{,}000$   & $0.24\%$ & $0.750$ & in-domain \\
NaturalQuestions& $\approx\!150$k & $1.15\%$ & --- & released \\
HotpotQA         & $\approx\!150$k & $9.06\%$ & --- & released \\
