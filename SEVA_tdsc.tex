...
loose---$12$--$18\times$ wider than the realized deviation at every $N$---and the three
measured deviations do not follow the rate it predicts (ratios $4.74$ and $1.47$ against the
$3.16$ of a $1/\sqrt{n}$ decay). We report the bound as the reason such a deviation must
shrink, and the measurements as what it does, without claiming the data exhibit the rate.

\emph{Scope.} This is a measurement at three corpus sizes, not a rate result: the deviation
is smaller at $10^{6}$ than at $10^{4}$, and that is the whole of the claim. The largest point
also uses a broader corpus (\S\ref{subsec:scale}), so we fit no exponent and do not extrapolate.
...
built for---clone cohesion averages $0.751$ against $\tau=0.841$, below the gate for
\emph{every} one of $50$ targets ($100\%$ poison-evasion), while $84\%$ remain retrieved at
$K=5$. Multiplicity rescues detection only partially and at the attacker's expense: $1$ to $5$
clones per target moves mean cohesion $0.722\to0.790$ and catch $0\%\to22.7\%$ (evasion
$100\%\to77.3\%$), because clones sharing a payload begin to resemble one another---the mutual
similarity the attack exists to avoid. The adversary's best configuration is the single
anchored clone, and that is the one the gate never sees. Nor does the complementary signal: nearest-neighbour
